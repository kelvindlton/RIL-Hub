...
// Static historical attendance logs for premium fidelity
...
function AttendanceContent() {
  const { events, profiles, currentUser, checkInUser } = useApp();
  ...
  const handleSelfCheckIn = async (e: React.FormEvent) => {
    ...
    if (passcode.trim() === targetEvent.qrCodeHash) {
      ...
      setCheckInResult({ success: false, message: 'Invalid check-in passcode. Please try again.' });
    }
  };

  // manual checkin for admin
  const handleManualCheckin = async (eventId: string, userId: string) => {
    const res = await checkInUser(eventId, userId, 'manual');
    alert(res.message);
  };

  // Filter profiles for manual check-in
  ...
  // Compile full history list (combining context events + static historical logs)
  ...
  // Helpers for category badge colors
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          <img src={p.avatar} alt={p.name} className="w-8 h-8 rounded-full border border-gray-200 shrink-0" />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
                  <div className="flex items-center gap-4">
                    <img
                      ...
                      className="w-14 h-14 rounded-full border-2 border-brand-blue bg-white shrink-0 object-cover"
                    />
                    <div>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm space-y-4">
                ...
                {checkInResult ? (
                  ...
                  <form onSubmit={handleSelfCheckIn} className="space-y-4">
                    ...
                    <button
                      type="submit"
                      className="w-full bg-brand-blue hover:bg-brand-blue/95 text-white text-xs font-bold py-2.5 rounded-xl transition-all shadow-sm flex items-center justify-center gap-1.5"
                    >
                          ...
                          <td className="py-3">
                            {row.status === 'present' ? (
                              ...
                              <div className="space-y-1">
                                ...
                                {/* Passcode Copy Helper for testing */}
                                <div className="flex items-center gap-1 text-[9.5px] font-bold text-gray-400 mt-1">
                                  <span>Code: <code className="bg-gray-100 px-1 py-0.2 rounded text-brand-blue">{row.qrCodeHash}</code></span>
                                  <button
                                    onClick={() => handleCopy(row.qrCodeHash || '', row.id)}
                                    className="text-brand-blue hover:text-brand-blue/80 font-bold text-[9px]"
                                  >
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {activeEvent.checkedInUsers.map((userId) => {
                            ...
                            return (
                              <div
                                key={userId}
                                className="flex items-center gap-1 px-2.5 py-0.5 bg-gray-100 border border-gray-200 rounded-full"
                                title={user.name}
                              >
                                <img src={user.avatar} alt={user.name} className="w-4 h-4 rounded-full border border-white" />
