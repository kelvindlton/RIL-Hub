...
function ChartsTab() {
  ...
  const avgPoints = Math.round(profiles.reduce((acc, p) => acc + p.points, 0) / totalMembers);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm space-y-2">
          <span className="text-[10px] uppercase font-bold text-gray-400 block">Total Cohort Registry</span>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm space-y-2">
          <span className="text-[10px] uppercase font-bold text-gray-400 block">Average Hub Score</span>
          <div className="flex justify-between items-end">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm space-y-2">
          <span className="text-[10px] uppercase font-bold text-gray-400 block">Active Streaks</span>
          <div className="flex justify-between items-end">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm space-y-2">
          <span className="text-[10px] uppercase font-bold text-gray-400 block">Turnout Headcount</span>
          <div className="flex justify-between items-end">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm space-y-4">
        ...
        <div className="h-52 flex items-end justify-around gap-2.5 px-4">
          {[
            { month: 'Jan', pct: 45 },
            { month: 'Feb', pct: 60 },
            { month: 'Mar', pct: 55 },
            ...
            <div key={month} className="flex flex-col items-center flex-1 h-full group">
              ...
              <div className="flex-1 w-full flex items-end min-h-0">
                <div
                  className={`w-full rounded-t-md transition-all duration-500 ${highlight ? 'bg-brand-blue shadow-md' : 'bg-brand-blue/10 border border-brand-blue/20 hover:bg-brand-blue'}`}
                  style={{ height: `${pct}%` }}
                />
              </div>
...
function DemographicsTab() {
  ...
  const skillTree = Object.entries(skillCounts)
    .map(([skill, count]) => ({ skill, count, percentage: Math.round((count / totalMembers) * 100) }))
  ...
  const deptTree = Object.entries(deptCounts)
    .map(([dept, count]) => ({ dept, count, percentage: Math.round((count / totalMembers) * 100) }))
    .sort((a, b) => b.count - a.count);
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-100 pb-4 mb-4">
          ...
          <button
            ...
            disabled={exporting}
            className="bg-brand-blue text-white text-xs font-bold px-5 py-2.5 rounded-lg hover:bg-brand-blue/90 disabled:opacity-50 transition-colors flex items-center justify-center gap-1.5 shadow-sm shrink-0"
          >
            {exporting ? (
        ...
        <div className="max-w-md mx-auto mt-12 bg-white rounded-lg border border-red-200 p-8 text-center space-y-4 shadow-md">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-2">
              The Reports & Analytics dashboard contains confidential community metric spreadsheets and operational growth charts. Your current persona ({currentUser.name}) does not have administrative permissions.
            </p>
          </div>
          <div className="bg-red-50/50 rounded-lg p-3 text-[10.5px] font-semibold text-red-600 border border-red-100 flex items-center justify-center gap-1.5">
            <ShieldCheck className="w-4 h-4 text-red-500" />
            <span>Switch to "Elena Rostova" in the top bar to preview Admin views!</span>
          </div>
        </div>
      </DashboardLayout>
    );
